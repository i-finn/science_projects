...
class VSlider(AxesWidget):
    ...
    def __init__(self, ax, label, valmin, valmax, valinit=0.5, valfmt='%1.2f',
        ...
        *slidermin* and *slidermax*
        ...
        valid property names (e.g., *axisbg*, *edgecolor*, *alpha*, ...)
        """
...
def on_key( event ):
    ...
    if event.key=="q" or event.key=="Q":
        ...
        x = plt.ginput(1)
        if refine_flag==1:
            check.circles[0].set_axisbg(axisbg_on)
            check.circles[1].set_axisbg(axisbg_off)
            check.circles[2].set_axisbg(axisbg_off)
            check.circles[3].set_axisbg(axisbg_off)
            check2.circles[0].set_axisbg(axisbg_on)
            check2.circles[1].set_axisbg(axisbg_off)
            check2.circles[2].set_axisbg(axisbg_off)
            check2.circles[3].set_axisbg(axisbg_off)
        ...
        if refine_flag==1 and peaklist!=[]:
            for line in peaklist:
                inten = line[1]
                freq = line[0]
                difference_list2.append((math.fabs(float(line[0])-float(x[0][0])),freq,inten))
                    
            difference_list2.sort()
            check2.labels[0].set_text(str(difference_list2[0][1])+" | "+str(difference_list2[0][2]))
            exp_label = str(difference_list2[0][1])+" | "+str(difference_list2[0][2])
            check2.labels[1].set_text(str(difference_list2[1][1])+" | "+str(difference_list2[1][2]))
        ...
        try:
            for z in picked_list:
                if difference_list[0][2]==z[1] and difference_list[0][3]==z[2]:
                    ...
                    pred_label = str(difference_list[0][2])+" | "+str(difference_list[0][3])+' | '+str(difference_list[0][1])+' | '+str(difference_list[0][4])
                    marker = 1
            ...
            for x in data:
                ...
                for y in picked_list:
                    if x[2]==y[1] and x[3]==y[2] and refine_flag==0:
                        s_b.append(0.0)
                        ...
                        t_b.append(x[0])
                        t_b.append(float(x[0])+0.0001)  
                    if x[2]==y[1] and x[3]==y[2] and refine_flag==1:
...
def func(label):
    """
    if refine_flag==1 and label!='':
        if  label==check.labels[0].get_text():
            
            #check2.lines[0][0].set_visible(True)
            #check2.lines[0][1].set_visible(True)
            check.lines[1][0].set_visible(False)
            check.lines[1][1].set_visible(False)
            check.lines[2][0].set_visible(False)
            check.lines[2][1].set_visible(False)
            check.lines[3][0].set_visible(False)
            check.lines[3][1].set_visible(False)
        if  label==check.labels[1].get_text():
            
            check.lines[0][0].set_visible(False)
            check.lines[0][1].set_visible(False)
            #check2.lines[1][0].set_visible(False)
            #check2.lines[1][1].set_visible(False)
            check.lines[2][0].set_visible(False)
            check.lines[2][1].set_visible(False)
            check.lines[3][0].set_visible(False)
            check.lines[3][1].set_visible(False)
        if  label==check.labels[2].get_text():
            
            check.lines[0][0].set_visible(False)
            check.lines[0][1].set_visible(False)
            check.lines[1][0].set_visible(False)
            check.lines[1][1].set_visible(False)
            #check2.lines[2][0].set_visible(False)
            #check2.lines[2][1].set_visible(False)  
            check.lines[3][0].set_visible(False)
            check.lines[3][1].set_visible(False)  
        if  label==check.labels[3].get_text():
            ...
            check.lines[2][0].set_visible(False)
            check.lines[2][1].set_visible(False)  
            #check2.lines[3][0].set_visible(False)
            #check2.lines[3][1].set_visible(False)    
        update_plot()
    ...
    try:
        ...
        for line in picked_list:
            if line[1]==check_select[0] and line[2]==check_select[1] and refine_flag==0:
                new_text+="\n removed: \n "+label
...
def func2(label):
    ...
    freq,inten = label.split(' | ')
    
    trans_2_plt.set_data(freq,inten)
    update_plot()
    """
    if  label==check2.labels[0].get_text():
        ...
        update_plot()
    if  label==check2.labels[1].get_text():
        
        check2.lines[0][0].set_visible(False)
        check2.lines[0][1].set_visible(False)
        #check2.lines[1][0].set_visible(False)
        #check2.lines[1][1].set_visible(False)
        check2.lines[2][0].set_visible(False)
        check2.lines[2][1].set_visible(False)
        update_plot()
    if  label==check2.labels[2].get_text():
...
def assign_pushed(event):
    check_select = pred_label.split(" | ")
    if exp_label=='Unassign Transition':
        counter =0
        for line in fit_list:
            if line[1]==check_select[0] and line[2]==check_select[1]:
    ...
    try:
        ...
        assign_freq = exp_label.split(" | ")[0]
        assign_inten = exp_label.split(" | ")[1]
        ...
        if counter==0:
            fit_list.append((check_select[2],check_select[0],check_select[1],check_select[3],assign_freq,assign_inten)) 
       ...
       # if marker == 0:
        #    new_fit_list.append((check_select[2],check_select[0],check_select[1],check_select[3],assign_freq))
            #new_text+="\n added: \n "+label
        #global fit_list
        #if refine_flag==0:
        #   text_box.set_text(new_text)
...
def run_main(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,dua,dub,duc,f_lower,f_upper,T,J_max,last_times_picked_list):
    ...
    global button
    global radio
    resetax = plt.axes([0.1, 0.025, 0.1, 0.05])
    button = Button(resetax, 'Reset Sliders', color='0.85', hovercolor='0.95')
    ...
    text_box2 = plt.text(-1,18, "Refine Mouse Selection:                             Select transitions by pushing 'q' and then clicking in the predicted spectrum ")
    #plt.show()

def initialize(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,f_lower,f_upper,T,J_max,last_times_picked_list):
    #matplotlib.use('TkAggx')
    global DJ_g,DJK_g,DK_g,ua_g,ub_g,uc_g,T_g,dJ_g,dK_g,f_lower_g,f_upper_g,J_max_g,fit_list
    J_max_g = J_max
    fit_list = []
    DJ_g = DJ
    DJK_g = DJK
    ...
    global peak_list_plt,exp_plt,trans_1_plt,trans_2_plt,trans_3_plt
    peak_list_plt, = ax2.plot([],[],'o',color='red')
    trans_1_plt, = ax2.plot([],[],'s',markersize=15,color='cyan')
    trans_2_plt, = ax2.plot([],[],'s',color='magenta')
    trans_3_plt, = ax2.plot([],[],'s',color='yellow')
    ax2.set_xlim([f_lower_g,f_upper_g])
    global locator   
...
def refine_main(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,dua,dub,duc,f_lower,f_upper,T,J_max,last_times_picked_list):
    ...
    global refine_flag
    refine_flag = 1
    global DJ_g,DJK_g,DK_g,ua_g,ub_g,uc_g,T_g,dJ_g,dK_g,f_lower_g,f_upper_g,J_max_g,triples_flag,fit_list
    fit_list = []
    triples_flag = 0
    J_max_g = J_max
    DJ_g = DJ
    ...
    ax.set_xlim([f_lower_g,f_upper_g])
    global picked_plt, ax2
    picked_plt, = plt.plot(t_b,s_b,'o',markersize=6,color='blue')
    triples_plt, = plt.plot([],[],lw=2,color='lime')
    #plt.axis([0, 1, -10, 10])
    #figure_h.canvas.mpl_connect('button_press_event', handle_mouse_press)
    ax2 = figure_h.add_subplot(211,sharex=ax)
    ax2.axes.get_xaxis().set_visible(False)
    ...
    global peak_list_plt,exp_plt,trans_1_plt,trans_2_plt,trans_3_plt
    peak_list_plt, = ax2.plot([],[],'o',color='red')
    trans_2_plt, = ax2.plot([],[],'o',markersize=6,color='blue')
    trans_1_plt, = ax2.plot([],[],'o',color='lime')
    ...
    global C_slider
    global rax
    rax = plt.axes([0.0, 0.2, 0.19, 0.3])
    global check
    global check2
    check = RadioButtons(rax, ('','','',''))
    rax2 = plt.axes([0.0, 0.6, 0.19, 0.3])
    check2 = RadioButtons(rax2, ('a','b','c','Unassign Transition'))
    check2.on_clicked(func2)
    check.on_clicked(func)
    
    global axisbg_on,axisbg_off
    axisbg_on = check.circles[0].get_axisbg()
    axisbg_off = check.circles[1].get_axisbg()
    ...
    global radio
    global fit_button
    fitaxes = plt.axes([0.0, 0.025, 0.1, 0.04])
    fit_button = Button(fitaxes, 'Assign', color=axcolor, hovercolor='0.5')

    fit_button.on_clicked(assign_pushed)
    
    
    
    resetax = plt.axes([0.1, 0.025, 0.1, 0.04])
    button = Button(resetax, 'Reset Sliders', color=axcolor, hovercolor='0.5')
