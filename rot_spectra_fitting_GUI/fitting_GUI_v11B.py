...
############################<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    def __init__(self, parent=None):
        ...
        self.proc_box.setText(str(cores_detected))
        ...
        self.refine_fit_button.setEnabled(False)
...
############################<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
	...
	#print fitting_GUI_B_v11.picked_list
	full_list = [(entry[3],entry[0],entry[1],entry[2],entry[4]) for entry in fitting_GUI_B_v11.picked_list] 
	if len(full_list)<3:
	    msg = "You need to choose at least three transitions before doing an automatic setup."
            ...
            counter = 0
            self.fit_list = []
            for entry in fitting_GUI_B_v11.picked_list:
                if entry[1]==best_option[1][1] and entry[2]==best_option[1][2]:
                    #print entry[1],best_option[1][1],entry[2],best_option[1][2]
                    fitting_GUI_B_v11.picked_list[counter] = (entry[0],entry[1],entry[2],entry[3],entry[4],'FIT')
                    ...
                    self.fit_list.append((entry[0],entry[1],entry[2],entry[3],entry[4]))
                else: fitting_GUI_B_v11.picked_list[counter] = (entry[0],entry[1],entry[2],entry[3],entry[4],'CHECK')
                counter+=1                  
            ...
            #fitting_GUI_B_v11.triples_plt.set_data([],[])
            try:
    ...
    def update_fit_check_status(self):
        ...
        for line in fitting_GUI_B_v11.picked_list:
            if line[5]=='CHECK':
                check_counter+=1
            if line[5]=='FIT':
                fit_counter+=1            
    ...
    def plot_triples(self):
        ...
        if self.plot_triples_cb.isChecked()==True:
            fitting_GUI_B_v11.triples_flag = 1
            fitting_GUI_B_v11.ax2.set_ylim([0,self.peaklist.max(0)[1]])    
            freq_high=float(unicode(self.f_upper_box.text()))
            freq_low=float(unicode(self.f_lower_box.text()))
            peak_1_uncertainty = float(unicode(self.uncertainty_box_1.text()))
            peak_2_uncertainty = float(unicode(self.uncertainty_box_2.text()))
            peak_3_uncertainty = float(unicode(self.uncertainty_box_3.text()))
            fitting_GUI_B_v11.peak_1_uncertainty = peak_1_uncertainty
            fitting_GUI_B_v11.peak_2_uncertainty = peak_2_uncertainty
            fitting_GUI_B_v11.peak_3_uncertainty = peak_3_uncertainty
            fitting_GUI_B_v11.peak_list = self.peaklist
            fitting_GUI_B_v11.freq_low = freq_low
            fitting_GUI_B_v11.freq_high = freq_high
            ...
            #fitting_GUI_B_v11.triples_plt.set_data(f_list,i_list)
            #fitting_GUI_B_v11.plt.show()

            #fitting_GUI_B_v11.plt.show()
            try:
                fitting_GUI_B_v11.trans_1 = qnum_list[0]
            except:
                pass
            try:
                fitting_GUI_B_v11.trans_2 = qnum_list[1]
            except: pass
            try:
                fitting_GUI_B_v11.trans_3 = qnum_list[2]
            except:
                pass
            self.update_triples_labels()
            self.update_triples_meter()
            
            fitting_GUI_B_v11.update_plot()
            I_1 = []
            F_1 = []
            I_2 = []
            F_2 = []
            ...
            for line in self.trans_3_peaks:
                F_3.append(line[0])
                I_3.append(line[1])
            #fitting_GUI_B_v11.trans_1_plt.set_data(F_1,I_1)
            #fitting_GUI_B_v11.trans_2_plt.set_data(F_2,I_2)
            #fitting_GUI_B_v11.trans_3_plt.set_data(F_3,I_3)
        ...
        if self.plot_triples_cb.isChecked()==False:
            ...
            fitting_GUI_B_v11.triples_flag = 0   
            fitting_GUI_B_v11.trans_1_plt.set_data([],[])
            fitting_GUI_B_v11.trans_2_plt.set_data([],[])
            fitting_GUI_B_v11.trans_3_plt.set_data([],[])       
            fitting_GUI_B_v11.triples_plt.set_data([],[])
            fitting_GUI_B_v11.plt.show()
    ...
    def update_exp_spectrum(self):
        ...
        try:
            if self.replot_flag==1:
                ...
                new_list_2 = list(OrderedDict.fromkeys(new_list_2))
                for line in new_list_2:
                ...
                fitting_GUI_B_v11.exp_plt.set_data(spectrum_list_F,spectrum_list_I)
                
                new_list.sort()
                fitting_GUI_B_v11.ax2.set_ylim([float(new_list[0]),float(new_list[-1])])
    ...
    def replot_button(self):
        ...
        if self.counter != 0:
            picked_list = fitting_GUI_B_v11.picked_list
        ...
        #fitting_GUI_B_v11.initialize(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,f_lower,f_upper,T,J_max,picked_list)  
        flag_1 = 0

         
        fitting_GUI_B_v11.run_main(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,duA,duB,duC,f_lower,f_upper,T,J_max,picked_list)
        try:
...
str(freq_low),str(inten_high),str(inten_low),str(temperature),str(Jmax),str(self.num_of_triples),check_peaks_str,str(trans_1),str(trans_2),str(trans_3))
        ...
        for line in fh_input:
                    ...
                    if line.split() != []:
                        ...
                        if line.split()[0] == "trans_1:" or line.split()[0] == "trans_2:" or line.split()[0] == "trans_3:":
                            ...
                            tuples = tuple(re_split)
                            print tuples
                            fitting_GUI_B_v11.picked_list.append((tuples[1],tuples[2],tuples[3],tuples[0],tuples[4],'FIT'))
                        ...
                        if fitting_peaks_flag == 1:
                            ...
                            tuples = tuple(re_split)
                            #print tuples
                            fitting_GUI_B_v11.picked_list.append((tuples[1],tuples[2],tuples[3],tuples[0],tuples[4],'CHECK'))
                        if line.split()[0] == "Check":
                            fitting_peaks_flag = 1
        #self.peaks_button_push()
        fh_input.close()
        self.peaks_button_push()
        for lin in fitting_GUI_B_v11.picked_list:
            print lin
        fitting_GUI_B_v11.update_plot()
        self.update_fit_check_status()
    ...
    def peak_pick_button(self):
        ...
        spectrum_2kHz = self.cubic_spline(fh_2,0.002) # Interpolates experimental spectrum to a 2 kHz resolution with a cubic spline.  Gives better peak-pick values.
        ...
        self.status_text.setStyleSheet('color: blue')
	self.refine_fit_button.setEnabled(True)
    ...
    def run_autofit(self):
        if os.path.isdir(unicode(self.job_box.text())):
            
            msg ='Job directory already exists. Please choose a new job name'
            QMessageBox.about(self, "Job directory error", msg.strip())
	...
	A = float(unicode(self.a_box.text()))
        ...
        u_B = float(unicode(self.ub_box.text()))
        u_C = float(unicode(self.uc_box.text()))
        fix_flags = [self.a_cb.isChecked(),self.b_cb.isChecked(),self.c_cb.isChecked(),self.DJ_cb.isChecked(),self.DJK_cb.isChecked(),self.DK_cb.isChecked(),self.dJ_cb.isChecked(),self.dK_cb.isChecked()]
	...
	GUI_working_directory = os.getcwd()
	#autofit_NS_success_flag = autofit_NS_module.autofit_NS(job_name,u_A,u_B,u_C,A,B,C,DJ,DJK,DK,dJ,dK,freq_high,freq_low,inten_high,inten_low,int(processors),temperature,Jmax,trans_1,trans_2,trans_3,check_peaks_list,self.peaklist,trans_1_peaks,trans_2_peaks,trans_3_peaks,fix_flags)
    ...
    def peaks_button_push(self):
        ...
        x = 0
        if fitting_GUI_B_v11.refine_flag==0:
            ...
            self.update_fit_check_status()
            #print fitting_GUI_B_v11.picked_list
        if fitting_GUI_B_v11.refine_flag==1:
            
            for entry in fitting_GUI_B_v11.fit_list:
                new_entry = entry
                item = QStandardItem(str(new_entry[1])+' | '+str(new_entry[2])+' | '+str(new_entry[0])+' | '+str(new_entry[3])+' Assign: '+str(new_entry[4])+' | '+str(new_entry[5]))
    ...
    def delete_button_push(self):
            if fitting_GUI_B_v11.refine_flag==0:
                counter = 0
                new_picked_list = []
                for x in fitting_GUI_B_v11.picked_list:
                    if self.model.item(counter).checkState()==0:
                        ...
                        pass
                    counter+=1                                                      
                fitting_GUI_B_v11.picked_list = new_picked_list    
            ...
            if fitting_GUI_B_v11.refine_flag==1:
                counter = 0
                new_picked_list = []
                for x in fitting_GUI_B_v11.fit_list:
                    print self.model.item(counter).checkState()
                    if self.model.item(counter).checkState()==0:
                        new_picked_list.append(x)
                        ...
                        pass
                    counter+=1     
                print 'newpickedlist:',new_picked_list                                          
                fitting_GUI_B_v11.fit_list = new_picked_list    
                print 'fitting_GUI_B_v11.fit_list',fitting_GUI_B_v11.fit_list
                ...
                self.lv.setModel(self.model) 
                self.model.itemChanged.connect(self.on_item_changed)   
                #fitting_GUI_B_v11.assign_pushed(True)
                s_a = []
                t_a = []
                for y in fitting_GUI_B_v11.fit_list:
                ...
                fitting_GUI_B_v11.trans_1_plt.set_data(t_a,s_a)
                fitting_GUI_B_v11.plt.draw()
    ...
    def update_triples_labels(self):
            ...
            for new_entry in fitting_GUI_B_v11.picked_list:
    ...
    def plot_current_peaklist(self):
        if self.peak_center_cb.isChecked()==True:
            ...
            fitting_GUI_B_v11.peak_list_plt.set_data(self.peaklist[:,0],self.peaklist[:,1]) 
            fitting_GUI_B_v11.peaklist = self.peaklist
            fitting_GUI_B_v11.ax2.set_ylim([0,self.peaklist.max(0)[1]])    
            fitting_GUI_B_v11.plt.show()          
        if self.peak_center_cb.isChecked()==False:
            
            fitting_GUI_B_v11.peak_list_plt.set_data([],[]) 
            fitting_GUI_B_v11.plt.show()  
...
#            self.c_box.setText(C)
	...
	self.uc_box.setText('1.0')
        """  
    def refine_fit_button_push(self):
        ...
        fitting_GUI_B_v11.refine_main(A,B,C,dA,dB,dC,DJ,DJK,DK,dJ,dK,ua,ub,uc,duA,duB,duC,f_lower,f_upper,T,J_max,picked_list)
        fitting_GUI_B_v11.peaklist = self.peaklist
        fitting_GUI_B_v11.ax2.set_ylim([0,self.peaklist.max(0)[1]])    
        fitting_GUI_B_v11.plt.show() 
        try:
            ...
            fitting_GUI_B_v11.plt.show()     
    def run_fit_button_push(self):
        if len(fitting_GUI_B_v11.fit_list)==0:
            msg = "Please select at least one transition to fit with the manual assignment button before running SPFIT"
            QMessageBox.about(self, "Fit Error", msg.strip()) 
            return
        old_fit_list = fitting_GUI_B_v11.fit_list
        A = float(unicode(self.a_box.text()))
        B = float(unicode(self.b_box.text()))
        C = float(unicode(self.c_box.text()))
        DJ = float(unicode(self.DJ_box.text()))
        ...
        if self.a_cb.isChecked()==True:
            a_uncert = '1.0E-025'
        if self.b_cb.isChecked()==True:
            b_uncert = '1.0E-025'            
        if self.c_cb.isChecked()==True:
            c_uncert = '1.0E-025'  
        if self.DJ_cb.isChecked()==True:
            DJ_uncert = '1.0E-25' 
        if self.DJK_cb.isChecked()==True:
            DJK_uncert = '1.0E-025'                       
        if self.DK_cb.isChecked()==True:
            DK_uncert = '1.0E-025'             
        if self.dJ_cb.isChecked()==True:
            dJ_uncert = '1.0E-025'     
        if self.dK_cb.isChecked()==True:
        ...
        input_file = ""
        input_file += "anisole                                         Wed Mar Thu Jun 03 17:45:45 2010\n"
        input_file += "   8  500   40    0    0.0000E+000    1.0000E+005    1.0000E+000 1.0000000000\n" # don't choose more than 497 check transitions or it will crash.
        input_file +="s   1  1  0  50  0  1  1  1  1  -1   0\n"
        input_file += "           10000  %s %s \n" % (A,a_uncert)
        input_file += "           20000  %s %s \n" % (B,b_uncert)
        input_file += "           30000  %s %s \n" % (C,c_uncert)
        input_file += "             200  %s %s \n" % (DJ,DJ_uncert)
        ...
        for line in fitting_GUI_B_v11.fit_list:
            
            #input_file += line[1]+' '+line[2]+'                        '+line[4]+' 0.50 1.0000\n' 
            
            input_file +=line[1][0:2]+' '+line[1][2:4]+' '+line[1][4:6]+' '+\
                      line[2][0:2]+' '+line[2][2:4]+' '+line[2][4:6]+'                      '+line[4]+' 0.50 1.0000\n' 


        fh_lin = open("default.lin", "w")
        ...
        try:
            ...
            a = subprocess.Popen("SPFIT0 default", stdout=subprocess.PIPE, shell=False)
            a.stdout.read()#used to let SPFIT finish            
            
        const_list = []

        fh_var = open("default.var")
        for line in fh_var:
            line1 = line.split()
            if line1[0] == '10000':
                new_A = line1[1]
            ...
            if line1[0] == '50000':
                new_dK = line1[1]                            
        fh_var.close()
	self.a_box.setText(new_A)
	self.b_box.setText(new_B)
	self.c_box.setText(new_C)
	self.DJ_box.setText(new_DJ)
	self.DJK_box.setText(new_DJK)
	self.DK_box.setText( new_DK)
	self.dJ_box.setText(new_dJ)
	self.dK_box.setText(new_dK)
        ...
        msg = """ Fit Completed, RMS=%s 
        """%rms_fit
        
        
        
        
        self.refine_fit_button_push()
        fitting_GUI_B_v11.fit_list = old_fit_list
        fitting_GUI_B_v11.assign_pushed_1()
        fitting_GUI_B_v11.peaklist = self.peaklist
        fitting_GUI_B_v11.ax2.set_ylim([0,self.peaklist.max(0)[1]])    
        fitting_GUI_B_v11.plt.show()   
        fitting_GUI_B_v11.update_plot()
        
        QMessageBox.about(self, "Fit Results", msg.strip())  
	...
	self.connect(self.uncertainty_box_3,SIGNAL("textEdited(QString)"),self.uncertainty_box_edit)
        ...
        self.draw_button = QPushButton("&Plot Input")
	...
	self.refine_fit_button = QPushButton("&Manual Assignment")
        self.connect(self.refine_fit_button, SIGNAL('clicked()'), self.refine_fit_button_push)      
 	self.run_fit_button = QPushButton("&Run SPFIT")
        self.connect(self.run_fit_button, SIGNAL('clicked()'), self.run_fit_button_push)           
	...
	hbox.addWidget(self.refine_fit_button,10,7)
	hbox.addWidget(self.run_fit_button,10,8)
	...
	hbox.addWidget(self.combo,8,9,10,11)
